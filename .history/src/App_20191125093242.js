  ...
  return (
    <div>
      <form>
        ...
        <input type="number" name="adults" placeholder="travelers" min="1" size="4" onChange={handleFindFlight} />
        ...
        <table className="table">
          ...
          <tbody>
            {state.map((flight, index) => {
              return (
                state.startDate, state.cabinClass, state.departure, state.destination, state.adults
                <tr key={index}>
                  <td>{flight.startDestination}</td>
                  <td>{flight.endDestination}</td>
                  <td>{flight.departure}</td>
